...
let VanillaCalendar = (function () {
    function VanillaCalendar(options) {
        ...
        const getFormatNumber = function (value) {
          if (value < 10) {
            return '0' + value;
          }
          return value;
        ...
        const getFormatData = function (value) {
          let now = new Date(value);
          return getFormatNumber(now.getDate()) + '.' + getFormatNumber(now.getMonth() + 1) + '.' + now.getFullYear();
        }
        const createDay = function (date) {
            ...
            if (opts.disabled.has(getFormatData(date))) {
        ...
        opts.calendar = document.querySelector(opts.selector);
        let infoDisabled = document.querySelector('.days-disabled');
        if(infoDisabled.dataset.disabled) {
          let disabledArr = infoDisabled.dataset.disabled.split(',');
          for (let i = 0; i <= disabledArr.length; i++) {
            opts.disabled.add(disabledArr[i]);
          } 
        };
        opts.calendar.innerHTML = `
            <div class="vanilla-calendar-header">
                <button type="button" class="vanilla-calendar-btn" data-calendar-toggle="previous"><svg class=\"icon widget-calendar__svg\"><use xlink:href=\"#arrow-left\"></use></svg></button>
                <div class="vanilla-calendar-header__label" data-calendar-label="month"></div>
                <button type="button" class="vanilla-calendar-btn" data-calendar-toggle="next"><svg class=\"icon widget-calendar__svg\"><use xlink:href=\"#arrow-right\"></use></svg><path d="M4,11V13H16L10.5,18.5L11.92,19.92L19.84,12L11.92,4.08L10.5,5.5L16,11H4Z"></path></svg></button>
            </div>
            <div class="vanilla-calendar-wrap">
